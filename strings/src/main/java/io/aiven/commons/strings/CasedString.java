...
public class CasedString {
	...
	public enum StringCase {
		 ...
		 * forces the first character to lower case. If specific capitalization rules
		 * are required use {@link WordUtils#capitalize(String)} to set the first
		 ...
		 *
		 * @param string
		 *            The string to split into segments.
		...
		public String[] getSegments(final String string) {
			...
			for (final char c : string.toCharArray()) {
				if (splitter.test(c)) {
					if (!preserveSplit || !builder.isEmpty()) {
