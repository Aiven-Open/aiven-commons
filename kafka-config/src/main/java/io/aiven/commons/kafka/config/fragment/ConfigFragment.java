...
/**
 * Config fragments encapsulate logical fragments of configuration that may be
 * used across multiple Connectors or across the source/sink of a connector
 * pair.
 * <p>
 * All implementing classes should be final. All configuration keys should be
 * specified as {@code static final String} within the fragment. They should be
 * package private so that test code can use them. All implementing classes must
 * implement a {@code public static ConfigDef update(final ConfigDef configDef)}
 ...
 * fragment is via the {@link #dataAccess} property. All access to configuration
 ...
 * the "foo" and "bar" values then something like {@code public Baz getBaz() {
 ...
 * {@link #validate(Map)} method may call the {@code validate} methods on the
...
public class ConfigFragment implements FragmentDataAccess {
	...
	/**
	 * Logs a deprecated message for a deprecated configuration key. This method
	 * should only be used when the key does not have a replacement and the
	 * documentation describes what to do.
	 *
	 * @param logger
	 *            the logger to log to.
	 * @param old
	 *            the deprecated configuration key.
	 * @param formatStr
	 *            the string to {@link String#format} the args.
	 * @param args
	 *            the arguments to format with the {@code formatStr}
	 */
	...
	/**
	 * Validate that the data in the configuration matches any restrictions other
	 * than those from the validator. Default implementation does nothing.
	 *
	 * @param configMap
	...
	/**
	 * Validate that the data in the configuration matches any restrictions. Default
