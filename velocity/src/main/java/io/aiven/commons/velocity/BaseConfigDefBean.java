 ...
 * from {@link ConfigDef.ConfigKey} instances. Also provides convenience methods
 * for formatting and ConfigKeyBean access.
 *
 * <p>
 * DEVHINT: Be careful when removing methods as this may invalidate contents and
 * functionality of the velocity templates.
 *
 * <p>
 * Example of use
 *
...
public class BaseConfigDefBean<T extends ConfigKeyBean> {
	 ...
	 *
	 * @param configDef
	 *            the Kafka configuration definition to process.
	 * @param constructor
	...
	/**
	 * Filters the {@code ConfigKey}s in the {@configDef} provided in the
	 * constructor.
	 *
	 * @param filter
	 *            A predicate to select keys from the ConfigDef.
	...
	private List<T> generatedFilteredList(Predicate<ConfigDef.ConfigKey> filter) {
		return configDef.configKeys().values().stream().filter(filter).map(constructor)
				.sorted(Comparator.comparing(ConfigKeyBean::getName)).collect(Collectors.toList());
	}
	 ...
	 *
	 * @param name
	 *            the name to find the parents for.
	...
	public List<T> dependents() {
		...
		configDef.configKeys().values().stream().filter(c -> c.dependents != null)
	...
	public List<T> configKeys() {
		return configDef.configKeys().values().stream().map(constructor)
				.sorted(Comparator.comparing(ConfigKeyBean::getName)).collect(Collectors.toList());
	}
