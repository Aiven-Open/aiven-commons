 ...
 * Since options can be added at various stages and by different libraries and because the "since" field
 * is to assist users in determining at what version an option became available in the final connector the SinceInfo
 * tracks the module and version where the option was added but provides a mechanism to override the display in the final
 * configuration.
 */
public class SinceInfo {

    /**
     * Record of the data to display.
     * @param groupId the groupId. May be {@code null}.
     * @param artifactId  the artifactId. May be {@code null}.
     * @param version the version.  Must be specified.
     */
    public record Data(String groupId, String artifactId, String version) implements Comparable<Data> {
        ...
        public int compareTo(Data other) {
            int result = StringUtils.defaultIfEmpty(groupId, "").compareTo(
                    StringUtils.defaultIfEmpty(other.groupId, ""));
            if (result == 0) {
                result = StringUtils.defaultIfEmpty(artifactId, "").compareTo(
                        StringUtils.defaultIfEmpty(other.artifactId, ""));
            }
            if (result == 0) {
                result = StringUtils.defaultIfEmpty(version, "").compareTo(
                        StringUtils.defaultIfEmpty(other.version, ""));
            }
            return result;
        }
    ...
    /**
     * Constructs the SinceInfo from the builder.  The builder must define {@code groupId}, {@code artifactId},
     * and {@code version}.
    ...
    /**
     * Sets the override for this SinceInfo. The builder must define {@code version}.  The
     * {@code groupId} and {@code artifactId} are optional and will be displayed if present.
     * @param builder the builder to define the override.
     */
     ...
     * If multiple ranges match the last one in the map will be applied.
     * @param overrideMap the map of OverrideRanges and builders to apply.
     */
    ...
    public static final class OverrideRange implements Comparable<OverrideRange> {
        ...
        /**
         * Creates an OverrideRange from a SinceInfo.Builder.  The version specified may be a range as defined in
         * the Maven method {@link VersionRange#createFromVersionSpec(String)}.
         * @param builder the Builder to extract the SinceInfo from.
         * @throws InvalidVersionSpecificationException on an invalid specification.
         * @see <a href="https://maven.apache.org/ref/3.9.7/apidocs/org/apache/maven/artifact/versioning/VersionRange.html#createFromVersionSpec(java.lang.String">
         *     VersionRange.createFromVersionSpec</a></a>
         */
         ...
         * @param sinceValue the SinceInfo to check.
